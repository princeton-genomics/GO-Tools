...
# GOID url
my $goidUrl = 'http://amigo.geneontology.org/amigo/tmp/<REPLACE_THIS>';
# Get organism gene url
my $geneUrl;
if ($annotationFile eq $annotationFileBase) {
    # not user-supplied annotation file
    $geneUrl = $metaData->organismGeneUrl($annotationFileBase, $opt_u); #Get organism gene url
#    $geneUrl =~ s/<REPLACE_THIS>//;
#    $geneUrl =~ s/<REPLACE_DBID>//;
    $DEBUG && print "\$geneUrl: $geneUrl\n";
} else {
    $geneUrl = "";
    $DEBUG && print "\$geneUrl undefined\n";
}
...
if ($opt_s) {
   ...
   if ($annotationFile ne $annotationFileBase) {
       $sysCall = "$map2slimPath ".$goSlimOntologyFile." $goDir"."/gene_ontology.obo"." ".$annotationFile." -o $outDir".$slimGeneAsscFileName;
   } else {
       $sysCall = "$map2slimPath ".$goSlimOntologyFile." $goDir"."/gene_ontology.obo"." $goDir"."/".$annotationFileBase." -o $outDir".$slimGeneAsscFileName;
   }
...
##############################################################################
   ...
   if (@inputDuplicates) {
       print TEXT "\nThese ",$#inputDuplicates+1," identifiers were duplicated in your input list: ", join(" ", @notFound), "\n";
   }
   if (@ambiguous) {
       print TEXT "\nThese ",$#ambiguous+1," identifiers were found to be ambiguous: ", join(" ", @ambiguous), "\n";
   }
   if (@notFound) {
       print TEXT "\nThese ",$#notFound+1," identifiers were found to be unannotated: ", join(" ", @notFound), "\n";
   }
   if (@{$lostRef}) {
       print TEXT "\nThese ",$#{$lostRef}+1, " identifiers were not annotated in the slim, but they had non-root annotations that were not in the slim: ", join(" ", @{$lostRef}), "\n";
   }
   if (@{$unannotatedRef}) {
...
#      $termLink = "<a target=\"infowin\" href=\"http://amigo.geneontology.org/cgi-bin/amigo/go.cgi?view=details&query=".$goid."\">".$term."</a>";
      $termLink = "<a target=\"infowin\" href=\"http://amigo.geneontology.org/amigo/term/".$goid."\">".$term."</a>";
...
#   }#end if ($noTermFoundMessage)

       print HTML "<br><b>Your input list contains ",$#inputGenes+1," genes.</b><br>";
       if (@inputDuplicates) {
	   print HTML "<br><b>These ",$#inputDuplicates+1," identifiers were duplicated in your input list:</b> ", join(" ", @inputDuplicates),"<br>";
       }
       if (@ambiguous) {
	   print HTML "<br><b>These ",$#ambiguous+1," identifiers were found to be ambiguous:</b> ", join(" ", @ambiguous),"<br>";
       }
       if (@notFound) {
	   print HTML "<br><b>These ",$#notFound+1," identifiers were found to be unannotated:</b> ", join(" ", @notFound), "<br>";
       }
       if (@{$lostRef}) {
	   print HTML "<br><b>These ",$#{$lostRef}+1, " identifiers were not annotated in the slim, but they had non-root annotations that were not in the slim:</b> ", join(" ", @{$lostRef}), "<br>";
       }
       if (@{$unannotatedRef}) {
...
##############################################################################
   ...
   foreach my $goid (keys(%totalNodeCounts)) {
      ...
      if ($goid ne 'GO:0008150' && $goid ne 'GO:0003674' && $goid ne 'GO:0005575' &&  $goid ne 'GO:0003673') {
         ...
         if (!$TERM) {
	     if (!$fullOntology) {
		 $fullOntology = GO::OntologyProvider::OboParser->new(ontologyFile=>$goDir."/gene_ontology.obo", aspect => $aspect);
	     }
...
##############################################################################
   ...
   my %ontologyAspect = ( P => 'biological_process',
...
e.g: perl GOTermMapper.pl B<-u> 'http://genome-www4.stanford.edu/cgi-bin/SGD/locus.pl?locus=' -a C -o goslim_yeast.2003 -g gene_association.sgd
...
The program default gene url(s) known for the genomes as of April 2004 are listed below:
            ...
            Saccharomyces Cerevisiae - http://db.yeastgenome.org/cgi-bin/SGD/locus.pl?locus=
